...
export function Navbar({ isConnected, onConnect, address }: NavbarProps) {
  ...
  return (
    <>
      ...
      >
        <div className="bg-white/95 backdrop-blur-xl rounded-2xl border border-slate-200/80 shadow-lg shadow-slate-200/30 px-3 sm:px-4 h-14 flex items-center justify-between gap-2 min-w-0">
          ...
          <Link href="/" className="flex items-center gap-2 group flex-shrink-0">
            ...
            <span className="font-bold text-slate-900 text-lg tracking-tight hidden sm:block whitespace-nowrap">Vortex</span>
          </Link>

          {/* Desktop Nav - Flexible, can shrink */}
          <div className="hidden md:flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl flex-1 justify-center min-w-0">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {/* Grant Metrics - Hidden on small screens */}
            <Link href="/grant-metrics" className="hidden lg:flex">
              <Button 
                variant="ghost" 
                size="sm"
                className="text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 h-9 px-2.5 rounded-lg transition-colors"
              >
                <BarChart3 className="w-4 h-4" />
                <span className="text-sm font-medium ml-1.5 hidden xl:inline">Metrics</span>
              </Button>
            </Link>
            ...
            <Button 
              ...
              size="sm"
              className={cn(
                "h-9 rounded-xl font-semibold transition-all shadow-sm whitespace-nowrap",
                isConnected 
                  ? "bg-slate-100 hover:bg-slate-200 text-slate-700 border-0 px-2.5 sm:px-3" 
                  : "bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white shadow-md shadow-indigo-500/20 px-3 sm:px-4"
              )}
            >
              {isConnected ? (
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0" />
                  <span className="hidden sm:inline">{address ? `${address.slice(0, 4)}...${address.slice(-3)}` : 'Connected'}</span>
            ...
            {/* Mobile Toggle */}
            <button 
              className="md:hidden p-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors flex-shrink-0"
