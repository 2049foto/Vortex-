...
export function Navbar({ isConnected, onConnect, address }: NavbarProps) {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-4xl mx-auto bg-white/95 backdrop-blur-xl rounded-2xl border border-slate-200/80 shadow-lg shadow-slate-200/30 px-4 h-14 flex items-center gap-4">
          {/* Logo */}
          ...
          {isConnected && (
            <div className="hidden md:flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl flex-1 max-w-md mx-auto">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.path}
                    href={link.path}
                    className="relative flex-1 px-2 py-2 rounded-lg text-sm font-medium transition-all text-center"
                  >
                    {active && (
                      <motion.div
                        layoutId="nav-pill"
                    ...
                    <span className={cn(
                      "relative z-10 flex items-center justify-center gap-1 transition-colors",
                      active ? "text-indigo-600 font-semibold" : "text-slate-500 hover:text-slate-700"
                    )}>
                      <link.icon className="w-4 h-4" />
                      <span className="hidden lg:inline text-xs">{link.name}</span>
                    </span>
                  </Link>
                );
              })}
          ...
          <div className="flex items-center gap-2 flex-shrink-0 ml-auto">
            {/* Grant Metrics */}
            <Link href="/grant-metrics" className="hidden lg:block">
              <Button 
                variant="ghost" 
                size="sm"
                className="text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 h-9 px-2 rounded-lg"
              >
            ...
            <Button 
              ...
              className={cn(
                ...
                isConnected 
                  ? "bg-slate-100 hover:bg-slate-200 text-slate-700 px-3" 
                  : "bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white shadow-md shadow-indigo-500/20 px-4"
              )}
            >
              {isConnected ? (
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-emerald-500" />
                  <span className="text-xs">{address ? `${address.slice(0, 6)}...${address.slice(-4)}` : '•••'}</span>
                </span>
              ) : (
                <span className="flex items-center gap-1.5">
